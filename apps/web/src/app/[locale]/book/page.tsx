    ...
    <div style={{ minHeight: '100vh', background: '#37353E', color: '#D3DAD9' }}>
      ...
      <main style={{ maxWidth: 1000, margin: '0 auto', padding: '6rem 1.5rem 4rem' }}>
        <div className="animate-fade-in-up" style={{ marginBottom: '2rem' }}>
          <h1 style={{ fontSize: '2rem', fontWeight: 900, letterSpacing: '-0.03em', color: '#D3DAD9' }}>Book a Session</h1>
          <p style={{ color: '#a3a69f', marginTop: '0.4rem' }}>Choose a day and time that works for you.</p>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1.4fr', gap: '1.5rem', alignItems: 'start' }}>
          ...
          <div className="glass-panel" style={{ borderRadius: '1.25rem', padding: '2rem', background: '#44444E', border: '1px solid rgba(211,218,217,0.12)' }}>
            <div style={{ width: 64, height: 64, borderRadius: '50%', background: '#37353E', border: '1px solid rgba(211,218,217,0.15)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1.6rem', fontWeight: 700, color: '#D3DAD9', marginBottom: '1.25rem' }}>E</div>
            <h2 style={{ fontSize: '1.2rem', fontWeight: 700, marginBottom: '0.25rem', color: '#D3DAD9' }}>Expert Mentor</h2>
            <p style={{ color: '#a3a69f', fontSize: '0.85rem', marginBottom: '1rem' }}>Industry veteran · Rating 4.9 / 5.0</p>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.6rem' }}>
              {[['Duration','60 minutes'],['Rate','$150/hr'],['Payment','Escrow — only released after session'],['Format','Live video + chat']].map(([label, val]) => (
                <div key={val} style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.85rem' }}>
                  <span style={{ color: '#a3a69f' }}>{label}</span>
                  <span style={{ color: '#D3DAD9', fontWeight: 500 }}>{val}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          {/* Calendar */}
          <div className="glass-panel" style={{ borderRadius: '1.25rem', padding: '2rem', background: '#44444E', border: '1px solid rgba(211,218,217,0.12)' }}>
            <h3 style={{ fontSize: '1rem', fontWeight: 700, marginBottom: '1rem', color: '#D3DAD9' }}>Select a Day</h3>
            <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap', marginBottom: '1.5rem' }}>
              {DAYS.map(d => (
                <button key={d} onClick={() => setSelectedDay(d)} style={{ padding: '0.5rem 0.9rem', borderRadius: '0.6rem', border: '1px solid', fontSize: '0.82rem', fontWeight: 500, cursor: 'pointer', fontFamily: 'inherit', transition: 'all 0.2s', ...(selectedDay === d ? { background: '#715A5A', borderColor: '#715A5A', color: '#D3DAD9' } : { background: '#37353E', borderColor: 'rgba(211,218,217,0.12)', color: '#a3a69f' }) }}>
                  {d}
                </button>
              ))}
            </div>

            <h3 style={{ fontSize: '1rem', fontWeight: 700, marginBottom: '1rem', color: '#D3DAD9' }}>Select a Time</h3>
